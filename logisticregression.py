...
class LogisticRegression:
    ...
    def train(self, inputs, outputs, learning_rate, epochs, *generate_2d_animation):
        ...
        for epoch in range(epochs): #for each epoch, we apply gradient descent for each training case:
            ...
            dloss_dexpoent = -2 * sum((outputs - predicted_outputs) * predicted_outputs * (1 - predicted_outputs))
            self.bias -= learning_rate * dloss_dexpoent * (-1)

            for index in range(self.coefficients.size): #we update each coefficient:

                dloss_dcoefficient = dloss_dexpoent * normalized_inputs[index] * (-1)
                self.coefficients[index] -= learning_rate * dloss_dcoefficient
    ...
    def test(self, inputs, expected_outputs):
        ...
        outputs = [self.predict(input) for input in inputs]
...
#model.test(np.array([1,2,3,4,5,6,7,8]), np.array([1,1,1,0,0,0,0,0]))
